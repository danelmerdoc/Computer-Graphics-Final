...
def add_mesh (polygons, mesh):
    #match type:#
    #if mesh ends in .obj, call add_obj
    #if mesh ends in .stl, call add_stl
            
    if mesh[-3:] == 'obj':
            realmesh = mesh.replace('obj', '')
            return add_obj(polygons, realmesh)
    elif mesh[-3:] == 'stl':
            realmesh = mesh.replace('stl', '')
            return add_stl(polygons, realmesh)
    else:
        print('file type not supported')
        return polygons
        
def add_obj (polygons, filename):
    file = open('obj/' + filename + '.obj').read().split('\n') 
    vertex = []
    vertexnorm = []
    face = []
    ...
    for line in file:
        if line[:2] == 'v ':
            vertex.append([float(line.split()[1]), float(line.split()[2]), float(line.split()[3])])
        if line[:3] == 'vn ':
            vertexnorm.append([float(line.split()[1]), float(line.split()[2]), float(line.split()[3])])
        if line[:2] == 'f ':
            face.append([int(line.split()[1].split('/')[0]), int(line.split()[2].split('/')[0]), int(line.split()[3].split('/')[0])])
    for f in face:
        add_polygon(polygons, vertex[f[0]-1][0], vertex[f[0]-1][1], vertex[f[0]-1][2], vertex[f[1]-1][0], vertex[f[1]-1][1], vertex[f[1]-1][2], vertex[f[2]-1][0], vertex[f[2]-1][1], vertex[f[2]-1][2])
    return polygons


def add_stl (polygons, filename):
    file = open('stl/' + filename + '.stl', encoding='utf-8').read().split('\n')
    vertex = []
    for line in file:
        if line[:12] == '      vertex':
            vertex.append([float(line.split()[1]), float(line.split()[2]), float(line.split()[3])])
    for i in range(len(vertex)):
        if i % 3 == 0:
            add_polygon(polygons, vertex[i][0], vertex[i][1], vertex[i][2], vertex[i+1][0], vertex[i+1][1], vertex[i+1][2], vertex[i+2][0], vertex[i+2][1], vertex[i+2][2])
